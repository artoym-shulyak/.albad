				...
				await User.findOne({ uuid }).then(async user => {
					if (new Date() < user.deadlines.battle.deadline) {
						await ctx[0].replyWithHTML(
							`🎯 Пᴏᴄᴧᴇ ᴨᴩᴇдыдыдущᴇᴦᴏ бᴏя ᴄʍᴏжᴇɯь ᴀᴛᴀᴋᴏʙᴀᴛь ʙ <strong>${user.deadlines.battle.deadline
							...
							{ uuid },
							{
								$set: {
									'deadlines.battle.deadline': new Date(
										Date.parse(new Date()) +
											(await _getRandomInt(1, user.deadlines.battle.term)) *
												60 *
												60 *
												1000
									),
			...
			case '🧩 баттл':
				...
				await User.findOne({ uuid }).then(async user => {
					if (new Date() < user.deadlines.battleChar.deadline) {
						await ctx[0].replyWithHTML(
							`🧩 Пᴏᴄᴧᴇ ᴨᴩᴇдыдыдущᴇᴦᴏ бᴏя ᴄʍᴏжᴇɯь ᴀᴛᴀᴋᴏʙᴀᴛь ʙ <strong>${user.deadlines.battleChar.deadline
								.toLocaleString('ru-RU')
								.split(',')[1]
								.trim()}</strong> (МСК).`
						)
							...
							{ uuid },
							{
								$set: {
									'deadlines.battleChar.deadline': await new Date(
										Date.parse(new Date()) +
											(await _getRandomInt(1, user.deadlines.battleChar.term)) *
												60 *
												60 *
												1000
									),
			...
			case '🧩 pvp команда':
				...
				await User.findOne({ uuid }).then(async user => {
					if (new Date() < user.deadlines.battleTeamCharacters.deadline) {
						await ctx[0].replyWithHTML(
							`🧩 Пᴏᴄᴧᴇ ᴨᴩᴇдыдыдущᴇᴦᴏ бᴏя ᴄʍᴏжᴇɯь ᴀᴛᴀᴋᴏʙᴀᴛь ʙ <strong>${user.deadlines.battleTeamCharacters.deadline
								.toLocaleString('ru-RU')
								.split(',')[1]
								.trim()}</strong> (МСК).`
						)
								...
								$set: {
									'deadlines.battleTeamCharacters.deadline': new Date(
										Date.parse(new Date()) +
											(await _getRandomInt(
												1,
												user.deadlines.battleTeamCharacters.term
											)) *
												60 *
												60 *
												1000
