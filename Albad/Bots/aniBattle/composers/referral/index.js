					...
					await ctx[0].editMessageText(
						...
						Extra.markup(
							Markup.inlineKeyboard([
								[Markup.callbackButton('Рᴇɸᴇᴩᴀᴧьнᴏᴇ ɜᴀдᴀниᴇ', `rf_task_${uuid}`)],
								[
				...
				await User.findOne({ uuid }).then(async user => {
					if (user.referral.stateReward) {
						await ctx[0].editMessageText(
							...
							Extra.markup(
								Markup.inlineKeyboard([
									Markup.callbackButton('⬅️ Нᴀɜᴀд', `referral_${uuid}`),
								])
							).HTML()
						)
						return
					...
					if (user.referral.completed) {
						await ctx[0].editMessageText(
							...
							Extra.markup(
								Markup.inlineKeyboard([
									Markup.callbackButton('⬅️ Нᴀɜᴀд', `referral_${uuid}`),
									Markup.callbackButton('🎁 Зᴀбᴩᴀᴛь', `rf_award_${uuid}`),
								])
							).HTML()
						)
					} else {
		 ...
		 case 'награда':
				...
				.then(async user => {
					...
					if (user.referral.completed === true) {
						...
						await sleep(1500)
						await ctx[0].reply('🎁 Ты ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ ⛓ "Рᴇɸᴇᴩᴀᴧьнᴏᴇ ɜᴀдᴀниᴇ".')
					}
				})
		  break
		 default:
