	...
	try {
		...
		if (ctx[0].updateType === 'message') {
		 await ctx[0].reply(ctx[0].i18n.t('donat'), {
				...
				...Extra.markup(
						Markup.inlineKeyboard([
							[Markup.urlButton('🥑 КИВИ',	 'qiwi.com/n/SELAVIES')],
							[Markup.callbackButton('',	 `pay_card_${uuid}`)],
							[Markup.urlButton('⚡️ Boosty',	 'https://boosty.to/anibattlegame/donate')],
							[Markup.urlButton('💭 Отправить чек',	 'https://t.me/ArtoymShulyak')],
						])
					).HTML()
				}
...
// Троттлинг
let throttleDonat = throttle(donat, 700)
