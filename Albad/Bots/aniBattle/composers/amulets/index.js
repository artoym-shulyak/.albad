		...
		switch (ctx[1]) {
				...
				await ctx[0].editMessageText(ctx[0].i18n.t('amulets'), 
					Extra.markup(
							Markup.inlineKeyboard([
								[Markup.callbackButton('🪬 Сучᴏнᴏᴋ', `amulet_first_${uuid}`)],
								[Markup.callbackButton('🎖 Диɜᴀнᴛ϶ᴩᴀ', `amulet_second_${uuid}`)],
								[Markup.callbackButton('🎭 Дᴇʍᴏнёнᴏᴋ', `amulet_third_${uuid}`)],
		 ...
		 case 1:
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('💰 Пᴏᴨᴏᴧниᴛь 195 ₽', `a_donat_${uuid}`)],
							[Markup.callbackButton('⬅️ Нᴀɜᴀд', `amulets_${uuid}`)],
						])
					).HTML()
				)
		 ...
		 case 2:
				await ctx[0].editMessageText(
						...
						Extra.markup(
							Markup.inlineKeyboard([
								[Markup.callbackButton('💰 Пᴏᴨᴏᴧниᴛь 890 ₽', `a_donat_${uuid}`)],
								[Markup.callbackButton('⬅️ Нᴀɜᴀд', `amulets_${uuid}`)],
							])
						).HTML()
					)
		 ...
		 case 3:
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('💰 Пᴏᴨᴏᴧниᴛь 1250 ₽', `a_donat_${uuid}`)],
							[Markup.callbackButton('⬅️ Нᴀɜᴀд', `amulets_${uuid}`)],
						])
					).HTML()
				)
...
// Троттлинг
let throttleAmulet = throttle(amulets, 700)
