		...
		if (ctx[1] === '1') {
			...
			.then(async user => {
				...
				if (user.quest.first.completed === true) {
					...
					await ctx[0].reply(`🎁 Уᴄᴨᴇɯнᴏ ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ 📜 Задание 1. `)
		...
		if (ctx[1] === '2') {
			...
			.then(async user => {
				...
				if (user.quest.second.completed === true) {
					...
					await ctx[0].reply(`🎁 Уᴄᴨᴇɯнᴏ ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ 📜 Задание 2. `)
					ctx[0].answerCbQuery(`🎁 Успешно`, {cache_time: 3})
				}
			})
			return
		...
		if (ctx[1] === '3') {
			...
			.then(async user => {
				...
				if (user.quest.third.completed === true) {
					...
					await ctx[0].reply(`🎁 Уᴄᴨᴇɯнᴏ ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ 📜 Задание 3. `)
					ctx[0].answerCbQuery(`🎁 Успешно`, {cache_time: 3})
				}
			})
			return
		...
		if (ctx[1] === '4') {
			...
			.then(async user => {
				...
				if (user.quest.fourth.completed === true) {
					...
					await ctx[0].reply(`🎁 Уᴄᴨᴇɯнᴏ ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ 📜 Задание 4. `)
					ctx[0].answerCbQuery(`🎁 Успешно`, {cache_time: 3})
				}
			})
			return
		...
		if (ctx[1] === '5') {
			...
			.then(async user => {
				...
				if (user.quest.fifth.completed === true) {
					...
					await ctx[0].reply(`🎁 Уᴄᴨᴇɯнᴏ ɜᴀбᴩᴀᴧ нᴀᴦᴩᴀду ɜᴀ 📜 Задание 5. `)
					ctx[0].answerCbQuery(`🎁 Успешно`, {cache_time: 3})
				}
			})
			return
...
// Троттлинг ----->
let questTaskPlayer = throttle(questTask, 1000)
let getAwardTaskQuestPlayer = throttle(getAwardTaskQuest, 1000)
