  ...
  switch (ctx[1]) {
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('📘 Ичᴀ Ичᴀ', `book_first_${uuid}`)],
							[
							...
							[Markup.callbackButton('📕 Тᴇᴛᴩᴀдь ᴄʍᴇᴩᴛи', `book_third_${uuid}`)],
   ...
   case '1':
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('💰 Куᴨиᴛь ɜᴀ 499 ₽', `b_donat_${uuid}`)],
							[Markup.callbackButton('⬅️ Нᴀɜᴀд', `books_${uuid}`)],
						])
					).HTML()
				)
   ...
   case '2':
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('💰 Куᴨиᴛь ɜᴀ 1295 ₽', `b_donat_${uuid}`)],
							[Markup.callbackButton('⬅️ Нᴀɜᴀд', `books_${uuid}`)],
						])
					).HTML()
				)
   ...
   case '3':
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('💰 Куᴨиᴛь ɜᴀ 1920 ₽', `b_donat_${uuid}`)],
							[Markup.callbackButton('⬅️ Нᴀɜᴀд', `books_${uuid}`)],
						])
					).HTML()
				)
...
// Троттлинг
let throttleBook = throttle(book, 700)
