  ...
  switch (ctx[1]) {
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('🎯 Бᴀᴛᴛᴧ', `rBPlayer_${uuid}`)],
							[Markup.callbackButton('💥 Бᴏᴇʙᴀя ʍᴏщь', `rCombatPower_${uuid}`)],
							[Markup.callbackButton('🧩 Бᴀᴛᴛᴧ', `rBChar_${uuid}`)],
							...
							[Markup.callbackButton('🏆 Гиᴧьдии', `rGClans_${uuid}`)],
   ...
   case '🎯 баттл':
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							Markup.callbackButton('⬅️ Нᴀɜᴀд', `rating_${uuid}`),
						])
					).HTML()
				)
	  		...
	  		await Rating.updateOne({uuid}, {$set: {"combatPower": user.stats.bm}})
				...
				await Rating.find()
					...
					.then(async users => {
						users.forEach((user, i) => {
							if (i < 10) {
								listUsers += `${i + 1}. ${user.name} ➠ ${roundNum(user.combatPower)}\n`
							}
							user.uuid == uuid ? (place = i + 1) : null
						})
					})
   ...
   case 'найм':
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							Markup.callbackButton('⬅️ Нᴀɜᴀд', `events_${uuid}`),
						])
					).HTML()
				)
