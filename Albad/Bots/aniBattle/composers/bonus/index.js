		...
		await User.findOne({ uuid }).then(async user => {
			if (new Date() < user.deadlines.bonus.deadline) {
				...
				await ctx.replyWithHTML(
					`🍀 Вᴏɜʙᴩᴀщᴀйᴄя ɜᴀ бᴏнуᴄᴏʍ ʙ <strong>${user.deadlines.bonus.deadline
				...
				if (
					...
					await User.updateOne(
						...
						await ctx.telegram.sendMessage(
							uuid,
							'Вᴩᴇʍя иᴄᴛᴋᴇᴧᴏ ➻ 📗 <strong>Solo Leaving</strong>.',
							{ parse_mode: 'HTML' }
						)
					})
				}
				...
				if (
					...
					await User.updateOne(
						...
						await ctx.telegram.sendMessage(
							uuid,
							'Вᴩᴇʍя иᴄᴛᴋᴇᴧᴏ ➻ 📘 <strong>Ичᴀ Ичᴀ</strong>.',
							{ parse_mode: 'HTML' }
						)
					})
				}
				...
				if (
					...
					await User.updateOne(
						...
						await ctx.telegram.sendMessage(
							uuid,
							'Вᴩᴇʍя иᴄᴛᴋᴇᴧᴏ ➻ 📕 <strong>Тᴇᴛᴩᴀдь ᴄʍᴇᴩᴛи</strong>.',
							{ parse_mode: 'HTML' }
						)
					})
				}
