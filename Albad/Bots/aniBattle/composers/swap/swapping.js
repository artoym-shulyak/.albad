				...
				await User.findOne({ uuid }).then(async user => {		
					if (user.resources.crio < ctx[1]) {
						...
						await User.updateOne(
							...
							await ctx[0].reply(
								`✅ Обʍᴇн: ${ctx[1]} 🧊 ➠ ${ctx[2]} 🃏 прошел успешно!`
							)
					}
				})
				...
				await Clan.findOne({ 'leader.uuid': uuid }).then(async clan => {
					...
					if (clan.money < ctx[1]) {
						...
						await Clan.updateOne(
							...
							await ctx[0].reply(
								`✅ Обʍᴇн: ${ctx[1]} 🪙 ➠ + ${ctx[2]} 👤 место в клане!`
							)
						})
					}
				})
						...
						await User.findOne({ uuid }).then(async user => {
							if (user.resources.crio < ctx[1]) {
								...
								if (user.resources.amulets.first) {
									await ctx[0].reply('Сучᴏнᴏᴋ 🪬 ужᴇ ʙ нᴀᴧичии ✅')
								} else {
									await User.updateOne(
										{ uuid },
										{
										...
										await ctx[0].reply(
											`✅ Обʍᴇн: ${ctx[1]} 🧊 ➠ Сучᴏнᴏᴋ 🪬 ᴨᴩᴏɯᴇᴧ уᴄᴨᴇɯнᴏ!`
										)
								}
							}
						})
						...
						await User.findOne({ uuid }).then(async user => {
							if (user.resources.crio < ctx[1]) {
								...
								if (user.resources.amulets.second) {
									await ctx[0].reply('Диɜᴀнᴛ϶ᴩᴀ 🎖 ужᴇ ʙ нᴀᴧичии ✅')
								} else {
									await User.updateOne(
										{ uuid },
										{
										...
										await ctx[0].reply(
											`✅ Обʍᴇн: ${ctx[1]} 🧊 ➠ Диɜᴀнᴛ϶ᴩᴀ 🎖 ᴨᴩᴏɯᴇᴧ уᴄᴨᴇɯнᴏ!`
										)
								}
							}
						})
						...
						await User.findOne({ uuid }).then(async user => {
							if (user.resources.crio < ctx[1]) {
								...
								if (user.resources.amulets.third) {
									await ctx[0].reply('Демонёнок 🎭 ужᴇ ʙ нᴀᴧичии ✅')
								} else {
									await User.updateOne(
										{ uuid },
										{
										...
										await ctx[0].reply(
											`✅ Обʍᴇн: ${ctx[1]} 🧊 ➠ Демонёнок 🎭 ᴨᴩᴏɯᴇᴧ уᴄᴨᴇɯнᴏ!`
										)
								}
							}
						})
