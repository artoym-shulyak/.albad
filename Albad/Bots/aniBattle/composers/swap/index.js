		...
		switch (ctx[1]) {
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton(' 95 🧊 ᴋᴩиᴏ ⇨  5 🃏', `s_c_first_${uuid}`)],
							[
							...
							[Markup.callbackButton(' 245 🧊 ᴋᴩиᴏ ⇨  30 🃏', `s_c_third_${uuid}`)],
							[
		 ...
		 case 'самоцветы':
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							[Markup.callbackButton('185 🧊 ᴋᴩиᴏ ⇨ 5 💎', `s_g_first_${uuid}`)],
							[Markup.callbackButton('340 🧊 ᴋᴩиᴏ ⇨ 15 💎', `s_g_second_${uuid}`)],
							[Markup.callbackButton('549 🧊 ᴋᴩиᴏ ⇨ 40 💎', `s_g_third_${uuid}`)],
							[
								Markup.callbackButton('🧊 ➠ 🔮', `swap_amulet_${uuid}`),
								Markup.callbackButton('🧊 ➠ 🃏', `swap_card_${uuid}`),
							],
				...
				await ctx[0].editMessageText(
					...
					Extra.markup(
						Markup.inlineKeyboard([
							...
							[
								Markup.callbackButton('⬅️ Нᴀɜᴀд', `shop_${uuid}`),
								Markup.urlButton('📝 Сᴨᴩᴀʙᴋᴀ', `https://telegra.ph/Lavka-Gildii-Komandy-10-19`),
							],
						])
					).HTML()
				)
...
// Троттлинг
let throttleSwapComponents = throttle(swapComponents, 700)
