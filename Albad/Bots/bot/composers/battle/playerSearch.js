									...
									{ uuid },
									{
										$set: {
											'deadlines.battle.deadline': new Date(
												Date.parse(new Date()) + _getRandomInt(1, user.deadlines.battle.term) * 60 * 60 * 1000
											),
										},
									}
								)
		  ...
		  case '🧩 баттл':
						...
						await User.findOne({ uuid }).then(async user => {
							if (new Date() < user.deadlines.battleChar.deadline) {
								await ctx[0].replyWithHTML(
									`🧩 После предыдыдущего боя сможешь атаковать <strong>${user.deadlines.battleChar.deadline
										.toLocaleString('ru-RU')
										.split(',')[1]
										.trim()}</strong> (МСК).`
								)
									...
									{ uuid },
									{
										$set: {
											'deadlines.battleChar.deadline': await new Date(
												Date.parse(new Date()) + _getRandomInt(1, user.deadlines.battleChar.term) * 60 * 60 * 1000
											),
										},
									}
								)
		  ...
		  case '🧩 pvp команда':
						...
						await User.findOne({ uuid }).then(async user => {
							if (new Date() < user.deadlines.battleTeamCharacters.deadline) {
								await ctx[0].replyWithHTML(
									`🧩 После предыдыдущего боя сможешь атаковать <strong>${user.deadlines.battleTeamCharacters.deadline
										.toLocaleString('ru-RU')
										.split(',')[1]
										.trim()}</strong> (МСК).`
								)
									...
									{ uuid },
									{
										$set: {
											'deadlines.battleTeamCharacters.deadline': new Date(
												Date.parse(new Date()) + _getRandomInt(1, user.deadlines.battleTeamCharacters.term) * 60 * 60 * 1000
											),
										},
									}
								)
...
// Actions
bot.action(new RegExp('fp_fight_(.+)'), ctx => throtteSearchFight([ctx, '🎯 баттл']))
bot.action(new RegExp('fp_char_fight_(.+)'), ctx => throtteSearchFight([ctx, '🧩 баттл']))
bot.action(new RegExp('fс_team_(.+)'), ctx => throtteSearchFight([ctx, '🧩 pvp команда']))
bot.action(new RegExp('info_attack_(.+)'), ctx => throtteSearchFight([ctx, 'инфо']))
